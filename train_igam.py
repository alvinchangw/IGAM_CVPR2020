...
import numpy as np
import sys
from model_new import Model, IgamConvDiscriminatorModel
...
def train(tf_seed, np_seed, train_steps, finetune_train_steps, out_steps, summary_steps, checkpoint_steps, step_size_schedule,
          ...
          sep_opt_version, grad_image_ratio, final_grad_image_ratio, num_grad_image_ratios, normalize_zero_mean, eval_adv_attack, same_optimizer, only_fully_connected, 
          finetuned_source_model_dir, train_finetune_source_model, finetune_img_random_pert, img_random_pert, only_finetune, finetune_whole_model, model_suffix, **kwargs):
    tf.set_random_seed(tf_seed)
    np.random.seed(np_seed)

    model_dir = model_dir + 'IGAM-%s_b%d_beta_%.3f_gamma_%.3f_disc_update_steps%d_l%dbc%d' % (dataset, train_batch_size, beta, gamma, disc_update_steps, disc_layers, disc_base_channels)  # TODO Replace with not defaults
    ...
    source_model = ModelExtendedLogits(mode='train', target_task_class_num=100, train_batch_size=train_batch_size)
    ...
    source_model_input_grad = tf.gradients(source_model.target_softmax, source_model.x_input, name="gradients_ig_source_model")[0]
    ...
    disc_model = IgamConvDiscriminatorModel(mode='train', dataset=dataset, train_batch_size=train_batch_size, num_conv_layers=disc_layers, base_num_channels=disc_base_channels, normalize_zero_mean=normalize_zero_mean,
        x_modelgrad_input_tensor=input_grad, y_modelgrad_input_tensor=labels_input_grad, x_source_modelgrad_input_tensor=source_model_input_grad, y_source_modelgrad_input_tensor=labels_source_model_input_grad, only_fully_connected=only_fully_connected)
    ...
    if finetune_whole_model:
        finetune_min_step = finetune_optimizer.minimize(source_model_finetune_loss, var_list=finetuned_source_model_vars)
    else:
        finetune_min_step = finetune_optimizer.minimize(source_model_finetune_loss, var_list=source_model_target_logit_vars)
    # Train classifier
    # classifier opt step
    ...
    with tf.Session() as sess:
        ...
        if train_finetune_source_model:
            ...
            for ii in tqdm(range(finetune_train_steps)):
                ...
                nat_dict = {source_model.x_input: x_batch, source_model.y_input: y_batch}
                ...
                if ii % summary_steps == 0:
                    ...
                    eval_dict = {source_model.x_input: x_eval_batch, source_model.y_input: y_eval_batch}
            ...
            finetuning_time_file_path = os.path.join(model_dir, 'finetuning_time.txt')
            with open(finetuning_time_file_path, "w") as f:
                f.write("Total finetuning time: {}".format(str(finetuning_time)))
        ...
        for ii in tqdm(range(train_steps)):
            ...
            nat_dict = {model.x_input: x_batch, model.y_input: y_batch, source_model.x_input: x_batch, source_model.y_input: y_batch}
            ...
            if ii % summary_steps == 0:
                ...
                eval_dict = {model.x_input: x_eval_batch, model.y_input: y_eval_batch, source_model.x_input: x_eval_batch, source_model.y_input: y_eval_batch}
...
if __name__ == '__main__':
    ...
    if args_dict['eval_adv_attack']:
        ...
        subprocess.run("python pgd_attack.py -d cifar100 --data_path datasets/cifar100 --attack_name fgsm --save_eval_log --num_steps 1 --no-random_start --step_size 8 --model_dir {} ; python run_attack.py -d cifar100 --data_path datasets/cifar100 --attack_name fgsm --save_eval_log --model_dir {} ; python pgd_attack.py -d cifar100 --data_path datasets/cifar100 --save_eval_log --model_dir {} ; python run_attack.py -d cifar100 --data_path datasets/cifar100 --save_eval_log --model_dir {} ; python pgd_attack.py -d cifar100 --data_path datasets/cifar100 --attack_name pgds5 --save_eval_log --num_steps 5 --model_dir {} ; python run_attack.py -d cifar100 --data_path datasets/cifar100 --attack_name pgds5 --save_eval_log --num_steps 5 --model_dir {} ; python pgd_attack.py -d cifar100 --data_path datasets/cifar100 --attack_name pgds20 --save_eval_log --num_steps 20 --model_dir {} ; python run_attack.py -d cifar100 --data_path datasets/cifar100 --attack_name pgds20 --save_eval_log --num_steps 20 --model_dir {}".format(model_dir, model_dir, model_dir, model_dir, model_dir, model_dir, model_dir, model_dir), shell=True)
