...
def get_args():
    ...
    parser.add("--source_model_dir", type=str, default="models/adv_trained", help="Path to load the source model checkpoints, default=models/adv_trained")
    parser.add("--data_dir", type=str, default="./datasets/tiny-imagenet/tiny-imagenet-200", help="Path to load datasets from, default=datasets")
    # parser.add("--dataset", "-d", type=str, default="cifar10", choices=["cifar10", "cifar100"], help="Path to load dataset, default=cifar10")
    parser.add("--dataset", "-d", type=str, default="tinyimagenet", choices=["cifar10", "cifar100", "tinyimagenet"], help="Path to load dataset, default=cifar100")
    ...
    parser.add("--pgd_steps", "-k", type=int, default=7, help="Number of steps to PGD attack, default=20")
    ...
    parser.add("--disc_layers", type=int, default=4, help="Number of conv layers in disc model, default=5")
    parser.add("--disc_base_channels", type=int, default=8, help="Number of channels in first disc conv layer, default=16")
    parser.add("--disc_avg_pool_hw", dest="disc_avg_pool_hw", action="store_true", help="Average pool channels before disc fc layer default=True")
    parser.add("--no-disc_avg_pool_hw", dest="disc_avg_pool_hw", action="store_false", help="Average pool channels before disc fc layer default=True")
    parser.set_defaults(disc_avg_pool_hw=True)
    parser.add("--steps_before_adv_opt", type=int, default=0, help="Number of training steps to wait before training adv loss, default=0")
    ...
    # parser.add("--grad_image_ratio", type=float, default=0.1, help="Ratio of input grad to mix with image default=0.1")
    ...
    parser.add("--finetune_img_random_pert", dest="finetune_img_random_pert", action="store_true", help="Random start image pertubation augmentation for finetuning default=False")
    parser.add("--no-finetune_img_random_pert", dest="finetune_img_random_pert", action="store_false", help="No random start image pertubation augmentation for finetuning default=False")
    parser.set_defaults(finetune_img_random_pert=False)
    parser.add("--finetune_train_steps", type=int, default=7812, help="Maximum number of training steps, default=7812")
    
    parser.add("--finetuned_source_model_dir", type=str, default="models/adv_trained_finetuned_on_tinyimagenet", help="Path to save/load the source model checkpoints, default=models/adv_trained_finetuned_on_cifar100")
    
    parser.add("--train_finetune_source_model", dest="train_finetune_source_model", action="store_true", help="Finetune source model to target task default=False")
    parser.add("--no-train_finetune_source_model", dest="train_finetune_source_model", action="store_false", help="No finetuning source model to target task default=False")
    parser.set_defaults(train_finetune_source_model=False)

    parser.add("--only_finetune", dest="only_finetune", action="store_true", help="Only finetune source model to target task wihtout IGAM training default=False")
    parser.set_defaults(only_finetune=False)

    parser.add("--model_suffix", type=str, default="", help="Suffix appended to model name")
    
    parser.add("--do_advtrain", dest="do_advtrain", action="store_true", help="Do adversarial training default=False")
    parser.add("--no-do_advtrain", dest="do_advtrain", action="store_false", help="No adversarial training default=False")
    parser.set_defaults(do_advtrain=True)

    parser.add("--do_advreg", dest="do_advreg", action="store_true", help="Do adversarial regularization default=True")
    parser.add("--no-do_advreg", dest="do_advreg", action="store_false", help="No adversarial regularization default=True")
    parser.set_defaults(do_advreg=False)
    ...
    parser.add("--model_type", type=str, default="", help="Model type, use 'source' for training model to transfer to other task")
